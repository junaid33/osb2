...
interface ProprietaryCarouselProps {
...
export default function ProprietaryCarousel({ proprietaryTools = [] }: ProprietaryCarouselProps) {
  const handleSoftwareClick = (toolSlug: string) => {
    // Link to page instead of manipulating URL params
    window.location.href = `/alternatives/${toolSlug}`
  }
          ...
          <div className="relative py-6 w-full">
            ...
            >
              {proprietaryTools.map((tool, index) => (
                <div
                  key={`${tool.name}-${index}`}
                  onClick={() => handleSoftwareClick(tool.slug)}
